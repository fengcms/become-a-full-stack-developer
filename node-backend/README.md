...
- 当前为「**功能验收通过、已正式冻结**」（tag `node-backend-v1.0`，2026-08-27 owner 裁决）：结构经 owner 确认符合预期、甚至「比我想的好」；主干转只读，后续 BUG 走增量维护（fix → 门禁复绿 → commit → 必要时 bump patch tag），不热改。下一步进入「写 M1 后端文章」（M1-01 ~ M1-30）。
