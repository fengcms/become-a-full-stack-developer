...
`meRoute@/api/v1/me`、`membersRoute@/api/v1/members`、`uploadRoute@/api/v1`。
行数（铁律 ≤200）：users 87 / users-admin 47 / me 119 / members 59 / upload 148 / attachment(lib) 61。
...
- 新增：`src/db/schema.ts`（attachments 表 + AttachmentRow）、`src/db/migrate.ts`（attachments DDL）、
  ...
  `src/routes/me.ts`、`src/routes/members.ts`、`src/routes/upload.ts`、`test/routes/users.test.ts`（14 例）。
- 改动：`src/app.ts`（挂载 5 条路由）、新增 `node-backend/.gitignore`（`/uploads/` 本地存储副作用不入库）。
...
|---|---|
| `tsc --noEmit` | ✅ 0 error |
| `biome check .` | ✅ 0 error / 0 warning（69 文件，2 info 非阻塞） |
| `vitest run` | ✅ 104 passed（15 文件；B5 行为级 14 例） |
| 契约结构门 `openapi-spec-validator` | ✅ `openapi.v1.yaml: OK` |
| 契约语义门 `check_contract.py` | ✅ 全部通过（33 OK 量级，未改动契约） |
