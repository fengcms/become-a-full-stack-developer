...
|---|---|---|---|
| `GET /api/v1/articles/{idOrSlug}/comments` | 可选鉴权（security: [{}, bearerAuth]） | listArticleComments | `routes/comments.ts` |
| `POST /api/v1/articles/{idOrSlug}/comments` | member（x-authz.minRole: member） | createComment | `routes/comments.ts` |
| `DELETE /api/v1/comments/{id}` | editor + ownerOverride（userId） + x-cascade: children | deleteComment | `routes/comments.ts` |
| `PATCH /api/v1/comments/{id}/status` | editor（x-authz.minRole: editor） | moderateComment | `routes/comments.ts` |
| `GET /api/v1/admin/comments` | editor（x-authz.minRole: editor） | listAdminComments | `routes/comments.ts` |

5 端点统一在 `commentsRoute`（Hono 实例）内定义，于 `app.ts` 以 `app.route('/api/v1', commentsRoute)` 挂载，内部用 `/articles/:idOrSlug/comments`、`/comments/:id`、`/comments/:id/status`、`/admin/comments` 区分，避免污染既有 articles 路由文件（严格遵守"不碰 articles.ts"边界）。
...
`ownerOverride` 通过 `loadComment` 中间件预载评论并 `c.set('comment', row)`，交给 `guard('editor', c => String(cm.userId))` 校验归属，缺失评论直接 404（非 403）。

## 三、文件清单

- 新增：`src/db/schema.ts`（comments 表 + CommentRow/CommentStatus）、`src/db/migrate.ts`（comments DDL）、`src/lib/comment.ts`（敏感词过滤/序列化/入参 schema）、`src/routes/comments.ts`（5 端点）、`test/routes/comments.test.ts`（10 例行为级测试）
- 改动：`src/app.ts`（挂载 commentsRoute）
...
## 五、门禁（待独立复验复跑）
> 注：本批提交时 Bash 工具临时不可用，门禁由总把控独立复验复跑（与 B3.5 同法：分三次独立调用）。
- `tsc --noEmit`：待跑（代码经逐文件人工核对，类型自洽）
- `biome check .`：待跑（无 any / 箭头函数 / @别名 / ≤200 行 / 无 ../）
- `vitest run`：待跑（B4 新增 10 例覆盖：默认态 / 敏感词 / 公开仅 approved / 401 / 未发布 404 / 改状态 / owner 删 / 他人 403 / 后台筛选 / 级联删除）
- 契约双门：结构 `openapi-spec-validator` + 语义 `check_contract.py` 均未触碰契约，预期零回归
- 代码行数：`comments.ts` ≈ 185 行（< 200）；`lib/comment.ts` ≈ 90 行；`schema.ts`/`migrate.ts` 仅为增量 DDL。
