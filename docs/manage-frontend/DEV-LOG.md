...
- 保留现有框架，落实写作保护、原状态保存、上传联动、批量审核、筛选恢复、评论上下文与页面文案等调整；细节见 [调整验收记录](./M2-页面细节调整验收-20260914.md)。
- 实测发现分类名称可能为空而分类 ID 有值，列表增加分类树名称兜底；小屏编辑器左右分栏过窄，改为默认单栏编辑。
- 类型检查、69 个测试与生产构建通过；源码 Biome 无错误，保留原有 5 个 CSS 警告与编辑器体积告警。使用独立临时数据库验收，未修改后端、契约或依赖，未提交 Git。
