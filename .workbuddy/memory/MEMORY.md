...
## 写作/维护分工（A 计划，2026-08-27 owner 裁定）
- **文章写作**：统筹 AI 负责（M0 已写、M1~M8 续写）。理由：上下文最多 + M0 建立的 voice/规范（标题体系、占位 token、内链、克制结构化文风、CTA、相关资源模块）须无缝延续；后端 AI 是 implementer 不擅教学写作，错配。
- **发布维护**（ARTICLES.md 镜像 + 链接回填 + 图片占位核验）：**M0 收尾由统筹 AI 顺手做**（需全局视图、量小）；**M1 起委派独立「发布维护 Agent」**，岗位说明书 `docs/发布维护-agent-岗位说明书.md`（统筹 AI 起草，owner 转交独立 AI）。
- **链接回填裁定**：正文 `{{LINK:Mx-yy}}` 占位统一在某里程碑（如 M0 全 8 篇）全部发完后一次性注入，不逐篇反复编辑已发布文章。
