/**
 * @file lib/request/core.ts
 * @description 请求内核：拼 URL、带令牌、拆统一信封 `{ code, message, data }`、401 静默刷新与重放。
 *
 * 设计对齐 M2 管理后台请求层，适配 Next.js：
 * - 仅用于**客户端组件**（带 token、互动请求）；RSC 直连 fetch 不经过此层。
 * - access token 存 Zustand 内存；refresh 走 HttpOnly Cookie（同源反代下浏览器自动携带）。
 *
 * ⚠️ 本文件是请求层高内聚核心，约 210 行。拆碎会让 rawRequest ↔ 刷新 ↔ 401 分流形成循环依赖，故整体保留。
...
import { API_BASE, ApiError, type ApiResponse } from '@/lib/request/errors'
import { forceLogout } from '@/lib/request/session'
import type { AuthResult } from '@/store/auth'
import { useAuthStore } from '@/store/auth'

/** 请求选项。在原生 RequestInit 基础上扩展 body / query / 三个 skip 开关。 */
export interface RequestOptions extends Omit<RequestInit, 'body'> {
  /** 请求体。普通对象自动 JSON 序列化；FormData 原样透传。 */
  body?: unknown
  /** query 参数。undefined / null / '' 的键自动丢弃。 */
...
/**
 * 同一时刻只允许一次刷新在飞。
 * 不做去重：页面首屏并发 N 个请求同时 401，就会打 N 次 /auth/refresh，
 * 而后端刷新令牌是**旋转**的——第一个换走令牌，其余全部失败，用户被莫名踢出。
 */
...
const performRefresh = async (): Promise<string> => {
  ...
  // 内存里没有 refreshToken，空体依赖后端 HttpOnly Cookie（同源代理下浏览器自动带）
  const auth = await rawRequest<AuthResult>(
    '/auth/refresh',
    { method: 'POST', body: {}, skipAuth: true, skipAuthRedirect: true, skipRefresh: true },
    { isRefreshCall: true },
  )
  if (generation !== useAuthStore.getState().generation) throw new Error('会话已结束，请重新登录')
...
const rawRequest = async <T>(
  ...
  if (response.ok && envelope.code === ErrCode.OK) {
    return envelope.data as T
  }

  const code = envelope.code as BizErrorCode
  ...
  if (response.status === 401) {
    ...
    if (canRetry) {
      try {
        ...
        if (!skipAuthRedirect && generation === useAuthStore.getState().generation)
          forceLogout('expired')
        throw refreshErr
      }
 ...
 * 失败是完全正常的路径（首次访问 / Cookie 过期），不弹错、不跳转。
 *
 * @returns 是否成功恢复会话。
 */
let bootInFlight: Promise<boolean> | null = null
/** Every StrictMode caller waits for the same initial refresh. */
export const bootstrapSession = (): Promise<boolean> => {
  if (bootInFlight) return bootInFlight
  const store = useAuthStore.getState()
  if (store.bootStatus === 'ready') return Promise.resolve(Boolean(store.accessToken))
  const generation = store.generation
  store.setBootStatus('booting')
  bootInFlight = refreshOnce()
    .then(() => true)
    .catch(() => {
      if (generation === useAuthStore.getState().generation) useAuthStore.getState().clear()
      return false
    })
    .finally(() => {
