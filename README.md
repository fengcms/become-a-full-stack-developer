...
1. 每篇文章对应一个 git tag，命名 `article/M1-05`；
2. 读者 `git checkout article/M1-05` 拿到该文发布时的完整代码状态；
3. 文章正文给出本篇涉及的 diff 范围（起始 tag → 结束 tag）；
4. `ARTICLES.md` 维护「标题 ↔ 链接 ↔ tag ↔ 代码位置」对照表；
...
| **M1 Node 后端** | 开发完成（53 路径 / 67 操作，对齐冻结契约）；结构调优完成（routes/services/shared/types 分层，126 测试 + 双门 33 OK 全绿）；当前为「功能验收通过、待调优（未冻结）」 |
| **M2–M8** | 未开始 |
