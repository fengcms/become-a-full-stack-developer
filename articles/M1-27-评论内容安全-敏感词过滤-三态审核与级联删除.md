...
![成为全栈·Node 后端篇·评论内容安全：敏感词过滤、三态审核与级联删除](https://i-blog.csdnimg.cn/direct/0c7bacfb383c4fa0b2d6938034103e70.png)
...
![敏感词过滤](https://i-blog.csdnimg.cn/direct/17203ba67f224d78a3d345df53400505.png)
...
![三态审核流](https://i-blog.csdnimg.cn/direct/f3ea8e9409c34a67a76845aa9ecf4d98.png)
...
分类删除是 `x-cascade:none`——有子节点就拒删，逼你先清依赖（见[分类树：无限级分类的存储、查询与环检测](https://blog.csdn.net/fungleo/article/details/164973360)的 P-28）。但**评论删除是 `x-cascade: children`**——直接把它的子回复（一层回复）一起删掉。为什么不同？因为分类是"结构性目录"，误删子树代价大、且用户通常不想丢数据；而评论的子回复是"对这条评论的回复"，父都没了，子回复挂空毫无意义，级联删反而更干净，避免孤儿回复。两条规则都合理，区别只在业务语义——**级联与否，永远由"数据还有没有独立存在的意义"决定**，不是拍脑袋。
...
下一篇（[辅助接口：相邻、相关、目录、统计与搜索](https://blog.csdn.net/fungleo/article/details/165111053)）我们聊"辅助接口"：相邻文章、相关推荐、目录 TOC、面包屑、全站统计、搜索这些"锦上添花"的端点，怎么在薄路由 + service 纪律下干净地长出来。
