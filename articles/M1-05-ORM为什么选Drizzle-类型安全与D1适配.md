...
![成为全栈·Node 后端篇·ORM 为什么选 Drizzle：类型安全与 D1 适配](https://i-blog.csdnimg.cn/direct/11cd314a64fc4451b34059271cf3c0aa.png)
...
![同一份 schema，本地和 Cloudflare 通吃](https://i-blog.csdnimg.cn/direct/9dd242695c274b32ad4492d786f26e4e.png)
...
- **DB 字段是 snake\_case（`created_at`），TS 属性是 camelCase（`createdAt`）**。这是项目铁律——数据库里用下划线命名，代码里用驼峰，转换在响应层统一做（还记得 M1-03 里 `toArticle` 干的事吗）。
