...
"阅读量 10086"这种数字，到底是人看的还是脚本刷的？如果一个接口每次被调就 `+1`，那阅读量毫无意义——爬虫、刷新、自己点几十下都能注水。这一篇对照我们 `node-backend` 真实的 `incrementViewCount`（`src/services/article.ts`），讲清阅读量防刷是怎么落地的：**去重 + 24 小时冷却 + 计数写分离**，以及并发和匿名身份这两个最容易被忽视的坑。
