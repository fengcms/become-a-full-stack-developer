...
![成为全栈·Node 后端篇·错误处理：异常分层与全局捕获](https://i-blog.csdnimg.cn/direct/dce8028172fc4ee69614be5d0f65f6a3.png)
...
![错误处理分层图](https://i-blog.csdnimg.cn/direct/7498ae8004744737a42544043a1dddc9.png)
...
数据库的唯一索引成了"最后一道防线"，冲突由 `catch` 接住并转成 `409 CONFLICT` 信封。**这既避免了先查后插的竞态，又不用在路由里散落 `any` 或字符串嗅探**——底层驱动的差异（better-sqlite3 与 D1 的错误码）被 `isUniqueConstraintError` 这一层收敛掉了。这个模式在注册登录 [注册登录全流程实现](https://blog.csdn.net/fungleo/article/details/164396193) 和文章 slug 唯一 [文章 CRUD 与投稿状态机](https://blog.csdn.net/fungleo/article/details/164453121) 里都会用到。
...
这也呼应了《工程公约》里"日志要规范、敏感信息要脱敏"的约定 [工程公约](https://blog.csdn.net/fungleo/article/details/164166250)——错误处理和日志，是同一套"对内透明、对外克制"哲学的两面。
...
下一篇（[参数校验：为什么必须在最外层做](https://blog.csdn.net/fungleo/article/details/164327423)）我们聊参数校验：为什么校验必须放在"最外层"（信任边界），以及 Zod 怎么把"所有外部输入不可信"这条安全意识落进代码。那是前端转后端最该建立的第一个安全直觉。
