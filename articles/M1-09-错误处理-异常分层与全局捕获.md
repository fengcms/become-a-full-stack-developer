...
数据库的唯一索引成了"最后一道防线"，冲突由 `catch` 接住并转成 `409 CONFLICT` 信封。**这既避免了先查后插的竞态，又不用在路由里散落 `any` 或字符串嗅探**——底层驱动的差异（better-sqlite3 与 D1 的错误码）被 `isUniqueConstraintError` 这一层收敛掉了。这个模式在注册登录 [注册登录全流程实现](https://blog.csdn.net/fungleo/article/details/164396193) 和文章 slug 唯一 {{LINK:M1-15}} 里都会用到。
