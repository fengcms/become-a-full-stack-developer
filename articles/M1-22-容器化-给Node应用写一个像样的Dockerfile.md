...
"在我机器上能跑"是软件工程最古老的笑话之一。你本地 `pnpm start` 一切正常，丢到测试服务器就报 `better-sqlite3` 找不到原生二进制；运维用老版本 Node 起服，顶层 `await` 直接语法报错。容器化要解决的，就是把"环境"也变成代码的一部分——一次构建，到处一致地跑。这一篇不堆 Docker 概念，而是对照我们 `node-backend` 真实冻结代码，写一个**真正能用**的 Dockerfile，并讲清每个决定背后的理由。
