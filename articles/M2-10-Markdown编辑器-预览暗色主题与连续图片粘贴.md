...
`React`、`Markdown编辑器`、`图片上传`、`异步编程`、`响应式设计`、`管理后台`
...
- 用途：文章封面；比例：16:9。
- 提示词：现代技术博客横版封面，中心为左右分栏 Markdown 编辑器，左侧正在输入文字并连续粘贴两张图片，右侧实时预览；上方有按顺序运行的上传队列，蓝绿色箭头保护最新正文不被覆盖。深蓝石墨背景，顶部短标题“编辑器难在异步边界”，扁平信息图风格，不出现品牌 Logo、水印、人物和乱码。

#### 2. `M2-10-异步插图时序`

- 插入位置：“异步上传会捕获一个过期的正文”小节之后；比例：16:9。
- 提示词：横向时序图，参与者为“用户”“受控正文”“latest ref”“图片上传”“编辑器”。用户先粘图启动上传，再继续输入形成新正文；上传完成后读取 latest ref，在新正文中插入图片并更新 ref。下方红色错误路径展示“闭包读取旧 value → 新输入被覆盖”。中文准确、深色背景、青绿正确路径、红色错误路径。

#### 3. `M2-10-宽窄屏编辑器`

- 插入位置：“实时预览要服从屏幕空间”小节之后；比例：16:9。
- 提示词：左右对比图，左侧宽屏管理后台采用“编辑 + 实时预览”双栏，右侧手机窄屏采用“专注编辑”单栏；中间标注断点“640px”。两边都展示同一段 Markdown 和暗色主题，布局整洁，中文标签清晰，深蓝灰界面、青绿色强调，不出现 Logo 和水印。

### 发布前核对

- [ ] Tag 为 6 个，简介不超过 250 个字符
- [ ] 三张配图已上传并替换占位符
- [ ] M1-18 链接再次核对，M2-09、M2-15 发布后回填内链
- [ ] latest ref、上传队列、10MB 限制与 640px 断点仍和源码一致
- [ ] 已删除本辅助区
