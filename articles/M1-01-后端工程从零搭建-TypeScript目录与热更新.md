...
- 测试策略本身是一套学问（[后端测试策略：单元、集成与测试数据库](https://blog.csdn.net/fungleo/article/details/164720486) 会展开：测试金字塔、用 `app.request` 发真实请求、测试库怎么隔离）。
- 除了这三道本地门禁，我们还有一道更特殊的——**契约门禁**：一份 OpenAPI 文档是「前后端唯一的接口真相源」，任何代码改动都不能偏离它（[接口文档自动化：让 OpenAPI 与代码不脱节](https://blog.csdn.net/fungleo/article/details/164584149) 会讲为什么文档会脱节、又怎么用双门校验守住）。
