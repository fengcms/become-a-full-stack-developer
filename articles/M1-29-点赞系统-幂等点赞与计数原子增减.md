...
- `29-封面`：幂等点赞——ON CONFLICT DO NOTHING + changes>0 才 +1，并发双发仅+1；扁平技术博客风、与专栏封面配色一致。
- `29-幂等点赞`：INSERT likes ON CONFLICT DO NOTHING → changes>0 才 like_count+1；并发双发仅+1 不500。
