...
上一篇我们把认证方案聊透了（JWT 访问令牌 + 有状态刷新令牌）。这一篇不再讲道理，直接把**注册、登录、刷新、登出**四条链路一行行走通，顺便解开一个很多人第一次做后端都会撞上的死锁："第一个管理员到底从哪来？"
...
- `13-注册登录流程图`：横向四步流程——①注册（bcrypt 哈希 + role=member + 查重兜底）→②登录（verifyPassword + 1001 不暴露账号存在）→③签发双令牌（access JWT 1h / refresh 存库可吊销）→④刷新/登出（旋转/revokeUserTokens 即时作废）。右下角画"无首 admin 死锁 → pnpm seed 破局"的小注释框。风格：扁平技术博客配图、配色与专栏封面一致、可放中文小标签。
