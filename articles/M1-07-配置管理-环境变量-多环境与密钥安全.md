...
这些变量"全部带默认值"，即便你什么都不改直接 `pnpm seed`，也能跑出一个可用 admin；生产环境再覆盖成强口令。把种子变量写进 `.env.example`，等于把"怎么造出第一个管理员"也变成了可复现的配置契约，而不是某个人脑子里的口头步骤。这套机制在注册登录 [注册登录全流程实现](https://blog.csdn.net/fungleo/article/details/164396193) 和部署上线 {{LINK:M1-23}} 两篇会真正用上。
