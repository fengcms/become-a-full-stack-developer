...
这也是《工程公约》里定下的规矩：密钥、本地数据库文件、构建产物一律不入库 {{LINK:M0-06}}。
...
- `07-配置分层图`：三层叠加金字塔——底层"代码默认值（zod default）"、中层"环境变量 .env"、顶层"运行时注入（CF binding）"，箭头向上表示"后者覆盖前者"；右侧画 `.env`（红字"不进库"）与 `.env.example`（绿字"进库=契约模板"）对照。风格：扁平技术博客配图、配色与专栏封面一致、可放中文小标签。
