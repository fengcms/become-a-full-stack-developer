...
这些变量"全部带默认值"，即便你什么都不改直接 `pnpm seed`，也能跑出一个可用 admin；生产环境再覆盖成强口令。把种子变量写进 `.env.example`，等于把"怎么造出第一个管理员"也变成了可复现的配置契约，而不是某个人脑子里的口头步骤。这套机制在注册登录 [注册登录全流程实现](https://blog.csdn.net/fungleo/article/details/164396193) 和部署上线 [部署上线：从本地起服到真正对外服务](https://blog.csdn.net/fungleo/article/details/164815866) 两篇会真正用上。
...
下一篇（[统一响应结构：HTTP状态码与业务码如何分工](https://blog.csdn.net/fungleo/article/details/164289071)）我们聊统一响应结构：为什么所有接口都该返回同一个"信封"格式，`code` / `message` / `data` / `requestId` / `timestamp` 各管什么，以及 HTTP 状态码和业务码怎么分工。那会引出我们踩过的一个真实坑——计划里的信封和契约里的信封打架，最终以契约为准。
