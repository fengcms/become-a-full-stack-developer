...
写代码时你 `pnpm start` 在本地跑通，不等于它能"对外服务"。部署上线的本质，是把"开发者机器上的一个能跑的进程"，变成"一台（或多台）服务器上、7×24 稳定对外、带 HTTPS、配置正确的服务"。这一篇对照我们 `node-backend` 的真实代码，讲清两条部署路线、以及环境变量这个"部署旋钮"到底怎么拧。
