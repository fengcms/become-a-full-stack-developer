...
上传是几乎每个内容产品都绕不开的需求：文章配图、附件、头像……但"收文件"这件看着简单的事，藏着去重、安全、双存储适配、删除一致性一串坑。这一篇把文件上传的完整链路拆开，重点讲我们怎么用一层"存储适配层"同时伺候 Cloudflare R2 和本地磁盘。它恰好把前面几篇讲的东西串了起来：最外层的信任边界校验（M1-10）、统一错误信封（M1-08/09）、以及为双部署预留的适配层（M1-24 详述）。
