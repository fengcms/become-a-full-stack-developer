...
![成为全栈·Node 后端篇·分类与标签：多对多关系的建模与查询](https://i-blog.csdnimg.cn/direct/bf6e39c69fdf4513a63d8ecac541c907.png)
...
![多对多怎么建：junction 表](https://i-blog.csdnimg.cn/direct/a8aebe71bd3d47f9a956afcab117149c.png)
...
![N+1 问题：别"查列表再逐条查标签"](https://i-blog.csdnimg.cn/direct/b622c430689b40c08d178ed007e2e1bf.png)
...
下一篇（[列表接口三件套：分页、筛选、排序](https://blog.csdn.net/fungleo/article/details/164425686)）我们进"列表接口三件套"：分页、筛选、排序。这里面藏着白名单防注入、`SCAN_LIMIT` 封顶、`ORDER BY` 必须显式限定基表（JOIN 后裸列会 `ambiguous column` 报 500）等一串实战细节。
