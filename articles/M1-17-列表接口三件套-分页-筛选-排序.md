...
凡是内容产品，几乎都有一个"列表页"：文章列表、评论列表、标签列表……列表接口看似简单，却是后端最容易写出性能事故和安全漏洞的地方。这一篇把列表接口的"三件套"——分页、筛选、排序——一次讲透，顺便把几个会让你半夜被报警叫醒的坑提前埋掉。

据不完全统计，一个内容类后端一半以上的流量、绝大多数慢查询，都来自列表接口——首页文章流、分类页、搜索、后台管理列表，全是一个"带条件取一页"的变体。所以把分页、筛选、排序这三件事做对、做安全，性价比极高，值得单独用一整篇来拆。
