...
但要注意**过度设计的红线**：在通知量不大时，加冗余字段和缓存纯属负担。当前 `COUNT` 直查就是对的——P-49 要传达的，是"知道这条查询在哪条规模线会吃紧，并把它当作明确的优化信号"，而不是现在就上重型方案。这和[辅助接口：相邻、相关、目录、统计与搜索的薄路由实现](https://blog.csdn.net/fungleo/article/details/165111053)相关文章"全量内存打分"的尺度判断是一脉相承的。
