...
![成为全栈·Node 后端篇·辅助接口：相邻、相关、目录、统计与搜索](https://i-blog.csdnimg.cn/direct/d5456566c9694c5ca86236ded41ae750.png)
...
这里 `orderBy(desc(articles.publishedAt))` 用**全限定列** `articles.publishedAt`——这正是我们[列表接口三件套：分页、筛选、排序](https://blog.csdn.net/fungleo/article/details/164425686)讲过的 P-11 排序纪律：当查询只涉及单表时写 `publishedAt` 也行，但养成"全限定基表列"的习惯，能在任何 JOIN 场景下根除 `ambiguous column` 错误。辅助接口虽简单，排序纪律照样不打折扣。

## 四、相关文章：共享标签 + 同分类打分

`getRelated` 是个"轻量推荐"：给当前文章算出最相关的几篇。真实实现不是上协同过滤模型，而是**规则打分**：

![相关文章打分](https://i-blog.csdnimg.cn/direct/563cd607b7b54d6eb145938715326b8d.png)
...
![目录 parseToc：一个纯函数的小艺术品](https://i-blog.csdnimg.cn/direct/af0766412cbe4b54aca32d70b8e2863f.png)
...
搜索的底层（LIKE 起步、SCAN_LIMIT 封顶、中文不分词的硬伤、升级 Meilisearch 的信号）我们在[全文搜索：从 LIKE 到全文索引](https://blog.csdn.net/fungleo/article/details/164453395)已经专门讲过，这里不再展开。值得补一句的是**排序纪律的延续**（P-11）：`searchArticles` 接收的 `?sort` 同样走[列表接口三件套：分页、筛选、排序](https://blog.csdn.net/fungleo/article/details/164425686)里那套"带符号字段 + `SORT_COLUMNS` 白名单"机制——只允许白名单内的字段、用 `-` 前缀表示降序，杜绝任意列排序带来的 SQL 注入与歧义列风险。辅助接口虽处在系统边缘，安全与纪律的底线一点没降。
...
下一篇（[点赞系统：幂等点赞与计数原子增减](https://blog.csdn.net/fungleo/article/details/165292499)）我们聊"点赞系统"：从"点赞数"这个字段，讲到怎么防止重复点赞、怎么和文章计数保持一致。
