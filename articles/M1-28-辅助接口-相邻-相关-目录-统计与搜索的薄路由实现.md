...
正文看完了，读者还想"下一篇看什么""这篇和哪篇相关""文章目录怎么跳"。这些**辅助接口**不直接是核心 CRUD，却是用户体验的黏合剂。这一篇对照我们 `node-backend` 真实的 `routes/aux.ts` 以及 `services/related.ts`、`shared/toc.ts`，看这些"锦上添花"的端点，如何在**薄路由 + service 纪律**下干净地长出来。
