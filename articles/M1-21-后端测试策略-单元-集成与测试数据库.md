...
![成为全栈·Node 后端篇·后端测试策略：单元、集成与测试数据库](https://i-blog.csdnimg.cn/direct/6eb3a94e1e1647a69db30ac61b83bcc0.png)
...
![测试金字塔](https://i-blog.csdnimg.cn/direct/8b042671385f4c188dc13931dbbc893d.png)
...
不启端口、不引额外依赖。这能成立，全靠 [统一响应结构：HTTP 状态码与业务码如何分工](https://blog.csdn.net/fungleo/article/details/164289071) 埋的伏笔：`shared/response.ts` 的信封构造器返回的是**原生 `Response.json()`**，而不是某个 Hono 专属对象——所以单测拿到 `Response` 就能直接 `res.json()` 断言，不需要起一个 Hono server 来"兜"着它。分层纪律在这里回馈了测试便利。相比 supertest 起端口、走真实 TCP，这种方式更快、也更不容易因为"端口被占用"之类环境问题而 flake（随机失败）。
...
`setup.ts` 在用例前建一个内存库、跑迁移、设为活跃。而具体的路由/服务测试（如 `articles.test.ts`）还会在每个用例前**再建一个新的 `:memory:`** 并 `migrate` + `setDb`：
...
![门禁全绿≠正确](https://i-blog.csdnimg.cn/direct/f34ca7d45d7043d0b1a578e96446177a.png)
...
还有一个更隐蔽的：阅读量防刷的"24 小时冷却窗口"（[阅读量防刷：去重、冷却与计数写分离](https://blog.csdn.net/fungleo/article/details/164973883) 会详讲），正常路径测试全绿，但**边界时刻——刚好跨过 24 小时窗口的那一瞬——没测**。结果上线后在这个边界踩了坑。这引出了下一点。
...
- **并发**：两个请求同时抢同一个 slug（唯一约束）、同时给一篇文章 +1 阅读量。我们的 `isUniqueConstraintError` 兜底（[错误处理：异常分层与全局捕获](https://blog.csdn.net/fungleo/article/details/164327333) 的 P-19）和阅读量原子 `±1`（[点赞系统：幂等点赞与计数原子增减](https://blog.csdn.net/fungleo/article/details/165292499)）这类并发逻辑，**必须有并发用例专门打**——单线程跑十遍都不会暴露的问题，并发一来就现形。顺带，并发测试不是"跑两次看谁赢"，而是用 `Promise.all` 同时发两个冲突请求，断言其中一个拿到 409、另一个成功——你要**设计**让冲突必然发生，才能验证兜底真的兜住了。盲跑十次可能每次都"恰好不冲突"，那测试等于没写。
...
下一篇（[容器化：给 Node 应用写一个像样的 Dockerfile](https://blog.csdn.net/fungleo/article/details/164721321)）我们聊"容器化"：给 Node 应用写一个像样的多阶段 Dockerfile——镜像怎么瘦、怎么非 root 跑。
