...
/** 业务错误码联合类型（排除 OK，成功码不需要错误文案）。 */
export type BizErrorCode = Exclude<(typeof ErrCode)[keyof typeof ErrCode], typeof ErrCode.OK>
 ...
 */
export const REFRESHABLE_CODES: readonly number[] = [ErrCode.TOKEN_INVALID, ErrCode.TOKEN_MISSING]

/** 必须立刻登出的码。 */
export const FORCE_LOGOUT_CODES: readonly number[] = [
 ...
 * @param code - 业务错误码。
 */
export const isRefreshable = (code: number): boolean => REFRESHABLE_CODES.includes(code)

/**
 * 该码是否必须立刻登出（刷新令牌失效 / 账号禁用）。
 * @param code - 业务错误码。
 */
export const shouldForceLogout = (code: number): boolean => FORCE_LOGOUT_CODES.includes(code)
