...
export const register = (data: RegisterRequest): Promise<AuthResult> =>
  request<AuthResult>('/auth/register', { method: 'POST', body: data, skipAuth: true })
...
export const login = (data: LoginRequest): Promise<AuthResult> =>
  request<AuthResult>('/auth/login', { method: 'POST', body: data, skipAuth: true })

/**
 * 登出。清后端 HttpOnly Cookie + 前端内存会话。
 */
export const logout = (): Promise<void> =>
  request<void>('/auth/logout', { method: 'POST', skipAuth: true, skipAuthRedirect: true })
